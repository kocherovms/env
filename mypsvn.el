;;; mypsvn.el --- Subversion interface for emacs
;; Copyright (C) 2002-2009 by Stefan Reichoer

;; Author: Stefan Reichoer <stefan@xsteve.at>
;; $Id: mypsvn.el 923804 2010-03-16 15:22:28Z cmpilato $

;; mypsvn.el is free software; you can redistribute it and/or modify
;; it under the terms of the GNU General Public License as published by
;; the Free Software Foundation; either version 2, or (at your option)
;; any later version.

;; mypsvn.el is distributed in the hope that it will be useful,
...
;; mypsvn.el is tested with GNU Emacs 21.3 on windows, debian linux,
;; freebsd5, red hat el4, ubuntu edgy with svn 1.4.0

;; mypsvn.el needs at least svn 1.1.0
;; if you upgrade to a higher version, you need to do a fresh checkout

;; mypsvn.el is an interface for the revision control tool subversion
;; (see http://subversion.tigris.org)
;; mypsvn.el provides a similar interface for subversion as pcl-cvs for cvs.
...
;; To use mypsvn.el put the following line in your .emacs:
;; (require 'psvn)
;; Start the svn interface with M-x svn-status

;; The latest version of mypsvn.el can be found at:
;;   http://www.xsteve.at/prg/emacs/mypsvn.el
...
;; TODO:
;; * shortcut for svn propset svn:keywords "Date" mypsvn.el
...
;; "svn-" is the package prefix used in mypsvn.el.  There are also longer
...
(defconst svn-psvn-revision "$Id: mypsvn.el 923804 2010-03-16 15:22:28Z cmpilato $"
...
This variable takes effect only when mypsvn.el is being loaded."
  :type 'boolean
  :group 'psvn)
...
line client does not output localized strings. mypsvn.el relies on the english
messages."
  :type '(repeat string)
  :group 'psvn)
...
(defvar svn-status-debug-level 0 "The mypsvn.el debugging verbosity level.
...
See mypsvn.el for an example function.")
...
;; 18.10.2004: the keymap parameter is not used (yet) in mypsvn.el
;;                           ,@(when keymap
;;                               `(mouse-face highlight
;;                                 local-map ,keymap)))
                         str))
...
(defvar svn-global-keymap nil "Global keymap for mypsvn.el.
...
(when (not svn-global-keymap)
  ...
  (define-key svn-global-keymap (kbd "f b") 'svn-status-via-bookmark)
  ...
  (define-key svn-global-keymap (kbd "d") 'svn-status-diff)
            ...
            (if run-asynchron
                (progn
                  ...
                  (let ((process-environment (svn-process-environment))
                    ...
                    ;; passphrase via stdio; mypsvn.el is currently unable
...
(defun svn-status-remove-temp-file-maybe ()
  "Remove any (no longer required) temporary files created by mypsvn.el."
...
(when (not svn-status-mode-map)
  ...
  ;; reasons: mypsvn.el may be loaded before user customizations, and
...
(defun svn-status-mode ()
  "Major mode used by mypsvn.el to display the output of \"svn status\".
...
(defun svn-status-bury-buffer (arg)
  "Bury the buffers used by mypsvn.el
Currently this is:
...
(defun svn-status-version ()
  "Show the version numbers for mypsvn.el and the svn command line client.
    ...
    (if (or (interactive-p) (not svn-status-cached-version-string))
        (progn
          ...
          (with-current-buffer svn-status-last-output-buffer-name
            ...
            (let ((buffer-read-only nil))
              (goto-char (point-min))
              (insert (format "mypsvn.el revision: %s\n\n" svn-psvn-revision)))
...
(defun svn-status-diff ()
  "Run `svn diff' on the current file."
  (interactive)
  (setq svn-status-diff-file-name (svn-status-line-info->filename (svn-status-get-file-information)))
  (svn-run nil t 'diff "diff" svn-status-diff-file-name)
  (split-window-vertically)
  (other-window 1)
  (switch-to-buffer svn-process-buffer-name)
  (goto-char (point-min)))
...
(if svn-log-edit-use-log-edit-mode
    (define-derived-mode svn-log-edit-mode log-edit-mode "svn-log-edit"
      "Wrapper around `log-edit-mode' for mypsvn.el"
    ...
    (let ((case-fold-search t))
      (if (search-forward "repository root: " nil t)
        ...
        (when (search-forward "repository uuid: " nil t)
          (message "mypsvn.el: Detected an old svn working copy in '%s'. Please check it out again to get a 'Repository Root' entry in the svn info output."
                   default-directory)
  ...
  (let ((file (concat (svn-status-base-dir) "++psvn.state")))
    (if (file-readable-p file)
        (with-temp-buffer
          ...
          (message "mypsvn.el: loaded %s" file))
      (if no-error
        ...
        (error "mypsvn.el: %s is not readable." file)))))
...
(defun svn-prepare-bug-report ()
  "Create the buffer *psvn-bug-report*. This buffer can be useful to debug problems with mypsvn.el"
  (interactive)
  (let* ((last-output-buffer-name (or svn-status-last-output-buffer-name svn-process-buffer-name))
    ...
    (delete-region (point-min) (point-max))
    (insert "This buffer holds some debug informations for mypsvn.el\n")
...
;; (load "/path/to/mypsvn.el")

;; Note the above will only work, if the loaded mypsvn.el has already the
;; function svn-prepare-for-reload
;; If this is not the case, do the following:
;; (load "/path/to/mypsvn.el");;make svn-prepare-for-reload available
;; (svn-prepare-for-reload)
;; (load "/path/to/mypsvn.el");; update the keybindings
;; --------------------------------------------------------------------------------
...
(defun svn-prepare-for-reload ()
  "This function resets some mypsvn.el variables to nil.
It makes reloading a newer version of mypsvn.el easier, if for example the used
